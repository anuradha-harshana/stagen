...
export function SupervisorQuestionsSkeleton() {
  return (
    <div className="flex flex-col gap-4 w-full font-sans">
      ...
      <div className="flex items-center justify-between pb-4 border-b border-blue-fantastic/10">
        <div className="flex items-center gap-3">
          ...
          <div className="space-y-1.5">
            <Skeleton className="h-6 w-60 bg-blue-fantastic/15" />
            <Skeleton className="h-4 w-80 bg-blue-fantastic/10" />
          </div>
        </div>
        <Skeleton className="h-9 w-48 rounded-2xl bg-blue-fantastic/10" />
      </div>
      ...
      {/* Filter bar skeleton */}
      <Skeleton className="h-12 w-full rounded-2xl bg-blue-fantastic/10" />

      {/* Questions list skeleton */}
      <div className="space-y-3">
        {Array.from({ length: 3 }).map((_, i) => (
          <Card key={i} className="bg-palladian border border-blue-fantastic/15">
            <CardContent className="p-5 space-y-4">
              <div className="flex items-center justify-between">
                <Skeleton className="h-5 w-48 bg-blue-fantastic/15" />
                <Skeleton className="h-6 w-24 rounded-full bg-blue-fantastic/10" />
              </div>
              <Skeleton className="h-4 w-full bg-blue-fantastic/10" />
              <Skeleton className="h-4 w-3/4 bg-blue-fantastic/10" />
              <div className="flex justify-end">
                <Skeleton className="h-8 w-32 rounded-xl bg-blue-fantastic/15" />
              </div>
            </CardContent>
          </Card>
        ))}
